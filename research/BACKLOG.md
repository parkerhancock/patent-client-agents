...
| **Yellow_byok connector queue (0)** | (no prior entry) | DPMA, CH/IPI, ES/OEPM, NZ/IPONZ, and TH/DIP now ship as contract-tested BYOK betas. OEPM uses exact-file CEO SOAP lookups; IPONZ uses its public OpenAPI and XSDs; Thailand DIP uses the official Data Exchange catalogue contract. All remain absent from the hosted demo until their terms permit shared proxy use. Live subscriber validation is still pending for the three 2026-08-03 additions. | (multiple) |
