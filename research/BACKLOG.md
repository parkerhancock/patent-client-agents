...
| **Yellow_byok connector queue (4)** | (no prior entry) | DPMA shipped as a mock-only beta on 2026-08-03. **4 offices remain yellow_byok with `connector_status: planned`**: CH/IPI (datadelivery API, ToU posted to Bern), ES/OEPM (5 free SOAP services, per-applicant credentials, Ley 37/2007 permissive), NZ/IPONZ (MBIE Azure APIM, RealMe + subscription key, separate from IP Australia), and TH/DIP (Data Exchange platform, 21 APIs, Bearer token via paper contract). Use the JPO/KIPO/INPI France pattern: env-gate MCP tools on per-user credentials and omit them from the hosted demo. | (multiple) |
