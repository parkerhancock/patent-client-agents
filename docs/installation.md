...
| Claude Code plugin (from GitHub marketplace) | Add 134 patent + trademark + adjacent-IP MCP tools to Claude Code with two slash commands; private/local credentialed deployments expose up to 232 tools | [§3](#3-claude-code-plugin-from-github) |
...
- `patent-client-agents-mcp` — launches the stdio MCP server (134 patent + trademark + adjacent-IP tools by default; up to 232 when every env-gated family is configured)
...
Use this when you use Claude Code and want the 134 patent + trademark +
adjacent-IP MCP tools dropped in with two slash commands. Private/local
deployments expose up to 232 tools when the corresponding credentials
...
4. On first MCP use, `uvx` fetches `patent-client-agents[mcp]` from
   **PyPI** (not from the cloned repo — the plugin manifest pins
   `uvx --from patent-client-agents[mcp]==0.25.0 patent-client-agents-mcp`)
...
Expect `patent-client-agents` with 134 tools by default. Local/private
servers expose up to 232 tools when all env-gated families are configured
...
|---|---|---|
| What it installs | MCP server only (134 default tools; up to 232 with all env-gated families configured) | Skill markdown for Python library usage |
...
Expect **134 tools** by default. Local/private servers expose up to
**232 tools** when every env-gated family is configured. Title starts
