...
| Claude Code plugin (from GitHub marketplace) | Add 106 patent + trademark + adjacent-IP MCP tools to Claude Code with two slash commands; private/local credentialed deployments expose up to 171 tools | [§3](#3-claude-code-plugin-from-github) |
...
- `patent-client-agents-mcp` — launches the stdio MCP server (106 patent + trademark + adjacent-IP tools by default; up to 171 when every env-gated family is configured)
...
Use this when you use Claude Code and want the 106 patent + trademark +
adjacent-IP MCP tools dropped in with two slash commands. Private/local
deployments expose up to 171 tools when the corresponding credentials
...
Expect `patent-client-agents` with 106 tools by default. Local/private
servers expose up to 171 tools when all env-gated families are configured
...
|---|---|---|
| What it installs | MCP server only (106 default tools; up to 171 with all env-gated families configured) | Skill markdown for Python library usage |
...
Expect **106 tools** by default. Local/private servers expose up to
**171 tools** when every env-gated family is configured. Title starts
