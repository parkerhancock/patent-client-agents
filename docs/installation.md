...
| Claude Code plugin (from GitHub marketplace) | Add 112 patent + trademark + adjacent-IP MCP tools to Claude Code with two slash commands; private/local credentialed deployments expose up to 169 tools | [§3](#3-claude-code-plugin-from-github) |
...
- `patent-client-agents-mcp` — launches the stdio MCP server (112 patent + trademark + adjacent-IP tools by default; up to 169 when every env-gated family is configured)
...
Use this when you use Claude Code and want the 112 patent + trademark +
adjacent-IP MCP tools dropped in with two slash commands. Private/local
deployments expose up to 169 tools when the corresponding credentials
...
4. On first MCP use, `uvx` fetches `patent-client-agents[mcp]` from
   **PyPI** (not from the cloned repo — the plugin manifest pins
   `uvx --from patent-client-agents[mcp]==0.23.0 patent-client-agents-mcp`)
...
Expect `patent-client-agents` with 112 tools by default. Local/private
servers expose up to 169 tools when all env-gated families are configured
...
|---|---|---|
| What it installs | MCP server only (112 default tools; up to 169 with all env-gated families configured) | Skill markdown for Python library usage |
...
Expect **112 tools** by default. Local/private servers expose up to
**169 tools** when every env-gated family is configured. Title starts
