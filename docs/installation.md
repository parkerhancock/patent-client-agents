...
| Claude Code plugin (from GitHub marketplace) | Add 116 patent + trademark + adjacent-IP MCP tools to Claude Code with two slash commands; private/local credentialed deployments expose up to 181 tools | [§3](#3-claude-code-plugin-from-github) |
...
- `patent-client-agents-mcp` — launches the stdio MCP server (116 patent + trademark + adjacent-IP tools by default; up to 181 when every env-gated family is configured)
...
Use this when you use Claude Code and want the 116 patent + trademark +
adjacent-IP MCP tools dropped in with two slash commands. Private/local
deployments expose up to 181 tools when the corresponding credentials
...
Expect `patent-client-agents` with 116 tools by default. Local/private
servers expose up to 181 tools when all env-gated families are configured
...
|---|---|---|
| What it installs | MCP server only (116 default tools; up to 181 with all env-gated families configured) | Skill markdown for Python library usage |
...
Expect **116 tools** by default. Local/private servers expose up to
**181 tools** when every env-gated family is configured. Title starts
