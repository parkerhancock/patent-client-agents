...
| Claude Code plugin (from GitHub marketplace) | Add 128 patent + trademark + adjacent-IP MCP tools to Claude Code with two slash commands; private/local credentialed deployments expose up to 199 tools | [§3](#3-claude-code-plugin-from-github) |
...
- `patent-client-agents-mcp` — launches the stdio MCP server (128 patent + trademark + adjacent-IP tools by default; up to 199 when every env-gated family is configured)
...
deployments expose up to 199 tools when the corresponding credentials
...
servers expose up to 199 tools when all env-gated families are configured
...
|---|---|---|
| What it installs | MCP server only (128 default tools; up to 199 with all env-gated families configured) | Skill markdown for Python library usage |
...
**199 tools** when every env-gated family is configured. Title starts
