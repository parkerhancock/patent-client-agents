...
| Claude Code plugin (from GitHub marketplace) | Add 131 patent + trademark + adjacent-IP MCP tools to Claude Code with two slash commands; private/local credentialed deployments expose up to 229 tools | [§3](#3-claude-code-plugin-from-github) |
...
- `patent-client-agents-mcp` — launches the stdio MCP server (131 patent + trademark + adjacent-IP tools by default; up to 229 when every env-gated family is configured)
...
Use this when you use Claude Code and want the 131 patent + trademark +
adjacent-IP MCP tools dropped in with two slash commands. Private/local
deployments expose up to 229 tools when the corresponding credentials
...
Expect `patent-client-agents` with 131 tools by default. Local/private
servers expose up to 229 tools when all env-gated families are configured
...
|---|---|---|
| What it installs | MCP server only (131 default tools; up to 229 with all env-gated families configured) | Skill markdown for Python library usage |
...
Expect **131 tools** by default. Local/private servers expose up to
**229 tools** when every env-gated family is configured. Title starts
