...
def _is_litigation_manifest_record(record: dict[str, Any]) -> bool:
    """Return whether a shipped-coverage row belongs in this catalog phase."""
    return record.get("category") == "adjudicative_records" or "litigation" in record.get(
        "data_types", []
    )


def load_litigation_manifest_records(
    path: Path = COVERAGE_MANIFEST_PATH,
) -> tuple[list[dict[str, Any]], list[str]]:
    """Load litigation and adjudicative rows from the shipped-coverage manifest."""
    try:
        payload = yaml.safe_load(path.read_text(encoding="utf-8"))
    except (OSError, yaml.YAMLError) as exc:
        return [], [f"{path.relative_to(ROOT)}: {exc}"]
    if not isinstance(payload, dict) or not isinstance(payload.get("sources"), list):
        return [], [f"{path.relative_to(ROOT)}: expected a top-level sources list"]
    records = [record for record in payload["sources"] if _is_litigation_manifest_record(record)]
    return records, []


def validate_litigation_manifest_parity(
    records: list[SourceRecord],
    manifest_records: list[dict[str, Any]] | None = None,
) -> list[str]:
    """Check that every shipped litigation row has a matching canonical record."""
    if manifest_records is None:
        manifest_records, errors = load_litigation_manifest_records()
        if errors:
            return errors

    errors: list[str] = []
    catalog_by_id = {record.id: record for record in records}
    for manifest_record in manifest_records:
        source_id = manifest_record.get("id")
        if not isinstance(source_id, str):
            errors.append("coverage/sources.yaml: litigation source is missing a string id")
            continue
        catalog_record = catalog_by_id.get(source_id)
        if catalog_record is None:
            errors.append(
                f"coverage/sources.yaml: litigation source missing from catalog: {source_id}"
            )
            continue

        jurisdiction = manifest_record.get("jurisdiction")
        if jurisdiction not in catalog_record.jurisdictions:
            errors.append(
                f"{catalog_record.path.relative_to(ROOT)}: jurisdictions do not include "
                f"coverage manifest jurisdiction {jurisdiction!r}"
            )

        manifest_module = manifest_record.get("connector", {}).get("module")
        if catalog_record.connector.get("status") != "shipped":
            errors.append(
                f"{catalog_record.path.relative_to(ROOT)}: manifest-backed connector must be shipped"
            )
        if catalog_record.connector.get("module") != manifest_module:
            errors.append(
                f"{catalog_record.path.relative_to(ROOT)}: connector.module differs from "
                f"coverage manifest ({manifest_module!r})"
            )

        catalog_rights = set(catalog_record.metadata.get("rights", []))
        missing_rights = sorted(set(manifest_record.get("rights", [])).difference(catalog_rights))
        if missing_rights:
            errors.append(
                f"{catalog_record.path.relative_to(ROOT)}: rights omit coverage manifest "
                f"value(s): {missing_rights}"
            )
    return errors
...
def main(argv: list[str] | None = None) -> int:
    ...
    errors.extend(validate_litigation_manifest_parity(records))
    if errors:
        for error in errors:
