"""Read-side API for the MOPP SQLite corpus.
...
class CorpusUnavailable(RuntimeError):
    """Raised when the MOPP corpus database cannot be located or opened."""
...
def _resolve_corpus_path(explicit: str | os.PathLike[str] | None) -> Path:
    ...
    env = os.environ.get("MOPP_CORPUS_PATH")
    if env:
        return Path(env)
...
_INSTALL_HINT = (
    "Run `patent-client-agents-build-mopp-corpus --output "
    "~/.cache/patent_client_agents/mopp.db` to build it, or set "
    "MOPP_CORPUS_PATH to an existing corpus file."
)
...
class CorpusDB:
    ...
    def open(
        ...
        if must_exist and not resolved.exists():
            raise CorpusUnavailable(f"MOPP corpus not found at {resolved}. {_INSTALL_HINT}")
        try:
            conn = sqlite3.connect(f"file:{resolved}?mode=ro", uri=True)
        except sqlite3.OperationalError as exc:
            raise CorpusUnavailable(
                f"Could not open MOPP corpus at {resolved}: {exc}. {_INSTALL_HINT}"
            ) from exc
