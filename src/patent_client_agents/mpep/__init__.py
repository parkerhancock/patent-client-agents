...
def get_corpus_status() -> CorpusStatus:
    """Return MPEP corpus freshness metadata for the validator and Provenance helper.

    Reads ``meta.source_version`` and ``meta.snapshot_date`` from the
    bundled SQLite corpus (see
    :mod:`patent_client_agents.mpep.corpus.schema`). Does not require a
    live upstream call — this is the callable
    ``scripts/build_coverage.py`` uses to detect drift, and the MPEP MCP
    tools use to stamp ``Provenance.corpus_synced_at`` /
    ``Provenance.corpus_version`` on every response (CONNECTOR_STANDARDS.md
    §4, §5.9).

    The corpus is located via ``MPEP_CORPUS_PATH`` or the local-dev
    default at ``~/.cache/patent_client_agents/mpep.db``. If the file is
    ...
    try:
        ...
        _logger.debug("MPEP corpus unavailable for get_corpus_status: %s", exc)
        ...
        _logger.warning("MPEP get_corpus_status: unexpected error reading corpus meta: %r", exc)
        return CorpusStatus(corpus_synced_at=None, corpus_version="unknown")
