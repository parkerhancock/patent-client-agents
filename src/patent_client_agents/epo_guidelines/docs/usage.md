...
`GUIDELINES_CORPUS_PATH` in the runtime env to point at it; the file does
not have to live in `~/.cache`.

## Search

* `GuidelinesClient.search(query, syntax="adj", sort="relevance",
...
* `GuidelinesClient.get_section(section)` accepts either a PA 1977 section
  ...
  or the full path `/guidance/manual-of-patent-practice-guidelines/...`).
* The returned `GuidelinesSection.html` is the cleaned outerHTML of the
  page's `<main>` element with form/sidebar/feedback widgets stripped.
  `.text` is the same content with whitespace collapsed.

## Versions

`GuidelinesClient.list_versions()` returns a single-entry list reflecting
...
If neither `GUIDELINES_CORPUS_PATH` nor `~/.cache/patent_client_agents/guidelines.db`
exists, the first call raises
`patent_client_agents.ukipo_guidelines.corpus.CorpusUnavailable` with the
