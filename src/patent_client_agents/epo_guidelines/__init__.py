"""Async API for the EPO Guidelines for Examination corpus without MCP wiring."""
...
class CorpusStatus(TypedDict):
    ...
    ``corpus_version`` mirrors the EPO Guidelines edition year as recorded
    in ``meta.guidelines_year`` — for the current corpus this is the EPO's
    annual Guidelines edition (e.g. ``"2024"`` for the March 2024 edition).
...
def get_corpus_status() -> CorpusStatus:
    """Return EPO Guidelines corpus freshness metadata for the validator + Provenance helper.

    Reads ``meta.guidelines_year`` and ``meta.snapshot_date`` from the
    bundled SQLite corpus (see
    :mod:`patent_client_agents.epo_guidelines.corpus.schema`). Does not
    ...
    The corpus is located via ``GUIDELINES_CORPUS_PATH`` or the local-dev
    default at ``~/.cache/patent_client_agents/guidelines.db``. If the
    file is missing, unreadable, or the schema is unexpected, returns
    ...
    try:
        ...
        _logger.debug("EPO Guidelines corpus unavailable for get_corpus_status: %s", exc)
        ...
        _logger.warning(
            "EPO Guidelines get_corpus_status: unexpected error reading corpus meta: %r",
            exc,
        )
        return CorpusStatus(corpus_synced_at=None, corpus_version="unknown")

    version = meta.get("guidelines_year") or "unknown"
...
def _parse_snapshot_date(value: str | None) -> datetime | None:
    ...
    try:
        ...
        _logger.debug("EPO Guidelines get_corpus_status: snapshot_date %r is not ISO date", value)
        return None
    return datetime(
        parsed_date.year,
        parsed_date.month,
