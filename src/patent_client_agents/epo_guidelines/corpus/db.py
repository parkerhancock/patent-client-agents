...
file produced by ``patent-client-agents-build-guidelines-corpus`` and serves
...
def _resolve_corpus_path(explicit: str | os.PathLike[str] | None) -> Path:
    ...
    env = os.environ.get("GUIDELINES_CORPUS_PATH")
    if env:
        return Path(env)
...
_INSTALL_HINT = (
    "Run `patent-client-agents-build-guidelines-corpus --output "
    "~/.cache/patent_client_agents/guidelines.db` to build it, or set "
    "GUIDELINES_CORPUS_PATH to an existing corpus file."
)
