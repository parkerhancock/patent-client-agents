"""Async API for the EPO Boards of Appeal Case Law corpus without MCP wiring."""
...
class CorpusStatus(TypedDict):
    ...
    ``corpus_version`` mirrors the EPO Case Law compendium edition year
    as recorded in ``meta.caselaw_year`` — for the current corpus this is
    the EPO Boards of Appeal "white book" edition year (e.g. ``"2022"``
    for the 10th edition). When the corpus is unbundled or unreadable the
...
def get_corpus_status() -> CorpusStatus:
    """Return EPO Case Law corpus freshness metadata for the validator + Provenance helper.

    Reads ``meta.caselaw_year`` and ``meta.snapshot_date`` from the
    bundled SQLite corpus (see
    :mod:`patent_client_agents.epo_case_law.corpus.schema`). Does not
    require a live upstream call — this is the callable
    ``scripts/build_coverage.py`` uses to detect drift, and the EPO
    Case Law MCP tools use to stamp ``Provenance.corpus_synced_at`` /
    ``Provenance.corpus_version`` on every response
    (CONNECTOR_STANDARDS.md §4, §5.9).

    The corpus is located via ``CASELAW_CORPUS_PATH`` or the local-dev
    default at ``~/.cache/patent_client_agents/caselaw.db``. If the file
    ...
    try:
        ...
        _logger.debug("EPO Case Law corpus unavailable for get_corpus_status: %s", exc)
        ...
        _logger.warning(
            "EPO Case Law get_corpus_status: unexpected error reading corpus meta: %r",
            exc,
        )
        return CorpusStatus(corpus_synced_at=None, corpus_version="unknown")

    version = meta.get("caselaw_year") or "unknown"
...
def _parse_snapshot_date(value: str | None) -> datetime | None:
    ...
    try:
        ...
        _logger.debug("EPO Case Law get_corpus_status: snapshot_date %r is not ISO date", value)
        return None
    return datetime(
        parsed_date.year,
        parsed_date.month,
