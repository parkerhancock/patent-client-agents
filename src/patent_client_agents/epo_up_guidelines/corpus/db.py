"""Read-side API for the GUIDELINES SQLite corpus.

The runtime never builds the corpus — it opens an already-built ``.db``
file produced by ``patent-client-agents-build-up-guidelines-corpus`` and serves
queries against it. Locator precedence:

1. ``UP_GUIDELINES_CORPUS_PATH`` env var (explicit, used in cloud deploys).
2. ``~/.cache/patent_client_agents/up_guidelines.db`` (local-dev convenience).
...
class CorpusUnavailable(RuntimeError):
    """Raised when the GUIDELINES corpus database cannot be located or opened."""
...
def _resolve_corpus_path(explicit: str | os.PathLike[str] | None) -> Path:
    ...
    env = os.environ.get("UP_GUIDELINES_CORPUS_PATH")
    if env:
        return Path(env)
...
_INSTALL_HINT = (
    "Run `patent-client-agents-build-up-guidelines-corpus --output "
    "~/.cache/patent_client_agents/up_guidelines.db` to build it, or set "
    "UP_GUIDELINES_CORPUS_PATH to an existing corpus file."
)
...
class CorpusDB:
    ...
    def open(
        ...
        if must_exist and not resolved.exists():
            raise CorpusUnavailable(f"GUIDELINES corpus not found at {resolved}. {_INSTALL_HINT}")
        try:
            conn = sqlite3.connect(f"file:{resolved}?mode=ro", uri=True)
        except sqlite3.OperationalError as exc:
            raise CorpusUnavailable(
                f"Could not open GUIDELINES corpus at {resolved}: {exc}. {_INSTALL_HINT}"
            ) from exc
