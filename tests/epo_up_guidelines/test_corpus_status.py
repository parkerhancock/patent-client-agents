"""Tests for the EPO Unitary Patent Guidelines ``get_corpus_status()`` callable.
...
from patent_client_agents.epo_up_guidelines import get_corpus_status


class TestGetCorpusStatusShape:
    def test_returns_dict_with_two_keys(self, up_corpus_env: Path) -> None:
...
class TestGetCorpusStatusFromBundledCorpus:
    def test_version_falls_back_to_up_guidelines_year(self, up_corpus_env: Path) -> None:
        """The UP scraper stamps ``up_guidelines_year`` (no ``source_version``).
        The callable surfaces the year as the corpus_version so the
        Provenance helper can quote it directly when warning about
        staleness (§4 + §5.13).
        """
        status = get_corpus_status()
        assert status["corpus_version"] == "2026"

    def test_synced_at_is_utc_datetime_normalized_to_midnight(self, up_corpus_env: Path) -> None:
        status = get_corpus_status()
...
class TestGetCorpusStatusMissingCorpus:
    def test_returns_unknown_when_corpus_missing(
        ...
        monkeypatch.setenv("UP_GUIDELINES_CORPUS_PATH", str(tmp_path / "missing.db"))
        status = get_corpus_status()
    ...
    def test_source_version_preferred_over_year(
        ...
        wins over the ``up_guidelines_year`` fallback.
        """
        from patent_client_agents.epo_up_guidelines.corpus.schema import DDL
        ...
        monkeypatch.setenv("UP_GUIDELINES_CORPUS_PATH", str(db_path))
        status = get_corpus_status()
        assert status["corpus_version"] == "2026-rev-2"
        assert status["corpus_synced_at"] == datetime(2026, 2, 20, tzinfo=UTC)
